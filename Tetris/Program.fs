...
module Game =
    ...
    let gameLoop gameContext =
        ...
        let fall gc distance =
            let fastDownGC = gc |> setl _y_position (gc.ActiveFigure.Position.y + distance)
            ...
            if isGroundedPosition fastDownGC.ActiveFigure fastDownGC.Map then
                ...
                let mapCompleteLine =
                    checkAndReturnLinesComplete newMap |> List.fold markLineToDelete newMap
        ...
        let playerMovedGC =
            ...
            | Left ->
                let movedLeftGC =
                    cleanGC |> setl _x_position (cleanGC.ActiveFigure.Position.x - playerSpeed)
            ...
            | Rotate ->
                let rotatedGC = cleanGC |> setl rotate ()
                returnChangedIfValid (isValidPosition rotatedGC.ActiveFigure rotatedGC.Map) cleanGC rotatedGC
            | FastDown -> fall cleanGC playerSpeed
            | NoAction -> cleanGC



        // let finalGC =
        fall playerMovedGC fallingSpeed |> setl _changeAction GameAction.NoAction
